...
func GetDefaultThemes() map[string]types.Config {
	...
	forestTheme.UI.NoConfirm = false
	...
	sunsetTheme.UI.NoConfirm = false
...
#animation = true         # Makes progress animations smooth when set at true
`
...
func loadConfig() (Config, error) {
	...
	config := Config{}
	config.Cache.Directory = filepath.Join(homeDir, ".cache", "vanish-test")
	config.Cache.Days = 10
	config.Logging.Enabled = true
	config.Logging.Directory = filepath.Join(homeDir, ".cache", "vanish-test", "logs")
